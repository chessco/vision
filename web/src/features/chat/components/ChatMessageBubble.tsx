...
import { UserCircle, Sparkles, Check, Download, Share2 } from 'lucide-react';
              ...
              <div className="mb-4">
                <div className="relative rounded-xl overflow-hidden border border-border-subtle group">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button className="p-2 rounded-lg bg-white/20 hover:bg-white/30 text-white backdrop-blur-sm"><Download className="w-5 h-5" /></button>
                    <button className="p-2 rounded-lg bg-white/20 hover:bg-white/30 text-white backdrop-blur-sm"><Share2 className="w-5 h-5" /></button>
                    <button className="p-2 rounded-lg bg-emerald-500/80 hover:bg-emerald-500 text-white backdrop-blur-sm px-4 font-medium flex items-center gap-2"><Check className="w-4 h-4" /> Approve</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {message.metadata?.suggestedActions && (
              <div className="mt-4 flex flex-wrap gap-2">
                {message.metadata.suggestedActions.map((action: string, i: number) => (
                  <button key={i} className="px-3 py-1.5 rounded-full bg-primary/10 hover:bg-primary/20 text-primary text-xs font-medium border border-primary/20 transition-colors">
                    {action}
                  </button>
                ))}
              </div>
