...
export function ContentStudioPage() {
  ...
  const tabs = [
    { id: 'workspace', label: t('Workspace'), icon: FileText },
    { id: 'humanizer', label: t('Humanizer'), icon: Wand2 },
    { id: 'comparison', label: t('Compare'), icon: Columns },
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6 animate-fade-in">
      <PageHeader
        ...
        description={t('AI-powered content production workspace. Generate, humanize, review, and publish across platforms.')}
        actions={
      ...
      {activeTab === 'workspace' && (
        <div className="space-y-6 stagger-children">
          ...
          <GlassPanel className="p-6 space-y-4">
            <h3 className="text-lg font-headings font-semibold text-white">{t('Content Brief')}</h3>
            <textarea
              value={brief}
              onChange={(e) => setBrief(e.target.value)}
              placeholder={t("Describe the content you want to create. e.g. 'Create a Facebook post promoting our new legal consulting service for small businesses. Tone should be professional yet approachable. Include a strong CTA.'")}
              rows={4}
            ...
            {selectedPlatforms.map(platform => (
              <GlassPanel key={platform} className="p-5 space-y-3" hover>
                ...
                <div className="flex gap-2 pt-2 border-t border-border-subtle">
                  <button className="flex-1 py-1.5 rounded-lg bg-primary/10 text-primary text-xs font-medium hover:bg-primary/20 transition-colors">{t('Humanize')}</button>
                  <button className="flex-1 py-1.5 rounded-lg bg-white/5 text-ink-muted text-xs font-medium hover:bg-white/10 hover:text-white transition-colors">{t('Edit')}</button>
                  <button className="flex-1 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 text-xs font-medium hover:bg-emerald-500/20 transition-colors">{t('Approve')}</button>
                </div>
              </GlassPanel>
            ))}
          </div>
      ...
      {activeTab === 'humanizer' && (
        <div className="space-y-6 stagger-children">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <GlassPanel className="p-6 space-y-4">
              <div className="flex items-center gap-2">
                <div className="px-2 py-1 rounded-md bg-red-500/10 text-red-400 text-[10px] font-labels font-semibold uppercase tracking-wider">{t('Before')}</div>
                <span className="text-xs text-ink-muted">{t('AI Generated')}</span>
              </div>
            ...
            <GlassPanel className="p-6 space-y-4" glow="cyan">
              <div className="flex items-center gap-2">
                <div className="px-2 py-1 rounded-md bg-emerald-500/10 text-emerald-400 text-[10px] font-labels font-semibold uppercase tracking-wider">{t('After')}</div>
                <span className="text-xs text-ink-muted">{t('Humanized')}</span>
              </div>
          ...
          {/* Scores */}
          <GlassPanel className="p-6 space-y-4">
            <h3 className="text-lg font-headings font-semibold text-white">{t('Content Scores')}</h3>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              <ScoreBar label={t('Human Score')} value={92} color="emerald" />
              <ScoreBar label={t('Emotion Score')} value={78} color="amber" />
              <ScoreBar label={t('Authenticity Score')} value={85} color="cyan" />
              <ScoreBar label={t('Brand Score')} value={88} color="primary" />
            </div>
          </GlassPanel>
        </div>
      )}
            ...
            {versions.map(v => (
              <GlassPanel key={v.id} className={clsx('p-4 space-y-3 cursor-pointer transition-all', v.selected ? 'border-primary/40 glow-purple' : '')} hover>
                <div className="flex items-center justify-between">
                  ...
                  {v.selected && <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{t('Selected')}</span>}
                </div>
                ...
                <button className={clsx(
                  ...
                  {v.selected ? t('✓ Selected') : t('Select')}
                </button>
              </GlassPanel>
            ))}
          </div>
      ...
      {activeTab === 'compliance' && (
        <GlassPanel className="p-6 animate-fade-in">
          <div className="space-y-4">
            <h3 className="text-lg font-headings font-semibold text-white">{t('Compliance Review')}</h3>
